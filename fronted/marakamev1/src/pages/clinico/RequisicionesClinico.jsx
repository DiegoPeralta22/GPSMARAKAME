...
export default function RequisicionesClinico() {
  ...
  const cargar = async () => {
    setCargando(true);
    try {
      const r = await fetch("http://localhost:3000/requisiciones?area=Clinico");
    ...
    try {
      const res = await fetch("http://localhost:3000/requisiciones", {
        ...
        body: JSON.stringify({
          ...
          nombre_solicitante: usuario.nombre || "Clínico",
          area: "Clinico",
          observaciones,
          total: totalCalc,
          items: validos,
    ...
    <div style={{ padding: "28px 32px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>Solicitudes enviadas por el área Clínica</p>
        </div>
    ...
    <div style={{ padding: "28px 32px", maxWidth: 860 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24, background: "#f9fafb", borderRadius: 10, padding: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 14, color: "#111827", fontWeight: 500 }}>Clínico</div>
        </div>
      </div>
